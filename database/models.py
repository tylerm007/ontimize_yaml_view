...
if (
    os.getenv("APILOGICPROJECT_NO_FLASK") is None
    or os.getenv("APILOGICPROJECT_NO_FLASK") == "None"
):
    Base = (
        SAFRSBaseX  # enables rules to be used outside of Flask, e.g., test data loading
    )
else:
...
class Application(Base):
    ...
    ApplicationEntityList: Mapped[List["ApplicationEntity"]] = relationship(
...
class Entity(Base):
    ...
    RuleConstraintList: Mapped[List["RuleConstraint"]] = relationship(
        back_populates="entity"
    )
    RuleDerivationList: Mapped[List["RuleDerivation"]] = relationship(
        foreign_keys="[RuleDerivation.as_child_entity]", back_populates="entity"
    )
    RuleDerivationList1: Mapped[List["RuleDerivation"]] = relationship(
        foreign_keys="[RuleDerivation.entity_name]", back_populates="entity1"
    )
    RuleEventList: Mapped[List["RuleEvent"]] = relationship(back_populates="entity")
    TabGroupList: Mapped[List["TabGroup"]] = relationship(
        foreign_keys="[TabGroup.entity_name]", back_populates="entity"
    )
    TabGroupList1: Mapped[List["TabGroup"]] = relationship(
        foreign_keys="[TabGroup.tab_entity]", back_populates="entity1"
    )
...
class ApplicationEntity(Base):
    ...
    application_name = Column(
        ForeignKey("application.name"), primary_key=True, nullable=False
    )
    ...
    application: Mapped["Application"] = relationship(
        back_populates=("ApplicationEntityList")
    )
...
class EntityAttr(Base):
    ...
    template_name = Column(
        ForeignKey("template.name"), server_default=text("'text'::character varying")
    )
    tooltip = Column(Text)
...
class RuleConstraint(Base):
    ...
    id = Column(
        Integer,
        server_default=text("nextval('rule_constraint_id_seq'::regclass)"),
        primary_key=True,
    )
...
class RuleDerivation(Base):
    __tablename__ = "rule_derivation"
    _s_collection_name = "RuleDerivation"  # type: ignore

    id = Column(
        Integer,
        server_default=text("nextval('rule_derivation_id_seq'::regclass)"),
        primary_key=True,
    )
    entity_name = Column(ForeignKey("entity.name"))
    ...
    entity: Mapped["Entity"] = relationship(
        foreign_keys="[RuleDerivation.as_child_entity]",
        back_populates=("RuleDerivationList"),
    )
    entity1: Mapped["Entity"] = relationship(
        foreign_keys="[RuleDerivation.entity_name]",
        back_populates=("RuleDerivationList1"),
    )
...
class RuleEvent(Base):
    ...
    id = Column(
        Integer,
        server_default=text("nextval('rule_event_id_seq'::regclass)"),
        primary_key=True,
    )
    entity_name = Column(ForeignKey("entity.name"))
...
class TabGroup(Base):
    ...
    # parent relationships (access parent)
    entity: Mapped["Entity"] = relationship(
        foreign_keys="[TabGroup.entity_name]", back_populates=("TabGroupList")
    )
    entity1: Mapped["Entity"] = relationship(
        foreign_keys="[TabGroup.tab_entity]", back_populates=("TabGroupList1")
    )
