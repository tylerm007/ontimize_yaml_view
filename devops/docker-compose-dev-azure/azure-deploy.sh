...
resourcename="ontimize"  # lower case, only
resourcegroup="ontimize_rg"
...
# git clone https://github.com/apilogicserver/ontimize.git
# cd ontimize
...
az group create --name ontimize_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group ontimize_rg --sku S1 --is-linux
az container create --resource-group ontimize_rg --name ontimize --image apilogicserver/ontimize:latest --dns-name-label ontimize --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//ontimize.westus.azurecontainer.io:5656'
